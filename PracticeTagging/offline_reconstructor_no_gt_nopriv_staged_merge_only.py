...
3) Train a heavy structured Offline Reconstructor with shared relpos transformer backbone and
   ...
   decomposition from hidden HLT generation labels.
...
CONFIG = {
    ...
    "loss": {
        ...
        "w_eff_floor": 0.45,
        "eff_floor_frac": 0.10,
        "w_sparse_split": 0.018,
        "w_sparse_gen": 0.020,
        "w_local": 0.05,
        "hard_select_alpha": 1.0,
        "unselected_penalty": 0.35,
        "gen_local_radius": 0.26,
        "merge_prior_radius": 0.06,
        "prior_density_kappa": 3.0,
        "prior_density_alpha": 2.0,
        "prior_split_alpha": 1.0,
        "prior_lowpt_alpha": 1.2,
        "prior_logit_bias": -0.10,
        "eff_low_pt_thr": 8.0,
    },
    "model": {
        "embed_dim": 128,
...
def _estimate_merge_ratio_prior(
    const_hlt: torch.Tensor,
    mask_hlt: torch.Tensor,
    action_prob: torch.Tensor,
    loss_cfg: Dict,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Weak, no-privileged merge-ratio prior from HLT geometry + model split tendency."""
    eps = 1e-8
    B, L, _ = const_hlt.shape
    ...
    dR = torch.sqrt(deta.pow(2) + dphi.pow(2) + eps)

    r0 = float(loss_cfg.get("merge_prior_radius", 0.06))
    kernel = torch.exp(-0.5 * (dR / max(r0, 1e-4)).pow(2))
    valid_pair = (mask_hlt[:, :, None] & mask_hlt[:, None, :]).float()
    eye = torch.eye(L, device=const_hlt.device).unsqueeze(0)
    valid_pair = valid_pair * (1.0 - eye)
    density_tok = (kernel * valid_pair).sum(dim=2)
    density_jet = (density_tok * mask_f).sum(dim=1) / (mask_f.sum(dim=1) + eps)

    kappa = float(loss_cfg.get("prior_density_kappa", 3.0))
    density_norm = density_jet / (density_jet + max(kappa, 1e-4))

    split_prob = (action_prob[..., 2] * mask_f).sum(dim=1) / (mask_f.sum(dim=1) + eps)

    low_thr = float(loss_cfg.get("eff_low_pt_thr", 8.0))
    low_frac = ((pt < low_thr).float() * mask_f).sum(dim=1) / (mask_f.sum(dim=1) + eps)

    alpha_d = float(loss_cfg.get("prior_density_alpha", 2.0))
    alpha_s = float(loss_cfg.get("prior_split_alpha", 1.0))
    alpha_l = float(loss_cfg.get("prior_lowpt_alpha", 1.2))
    b0 = float(loss_cfg.get("prior_logit_bias", -0.1))
    prior_logit = b0 + alpha_d * density_norm + alpha_s * split_prob - alpha_l * low_frac
    merge_ratio_prior = torch.sigmoid(prior_logit).clamp(0.05, 0.95)
    return merge_ratio_prior, low_frac


def _build_hard_selection_mask(
    out: Dict[str, torch.Tensor],
    mask_hlt: torch.Tensor,
    max_tokens: int | None = None,
) -> torch.Tensor:
    """
    Build inference-like hard selection mask (0/1) using predicted branch budgets.
    This mirrors reconstruct_dataset branch-aware top-k behavior.
    """
    ...
    idx_child = torch.arange(L, L + n_child, device=device)
    idx_gen = torch.arange(gen_start, gen_start + n_gen, device=device)

    for i in range(B):
        max_cap = N if max_tokens is None else int(min(N, max_tokens))
        k_budget = int(torch.clamp(torch.round(out["budget_total"][i]), 1.0, float(max_cap)).item())
        valid_tok = torch.where(mask_hlt[i])[0]
        if valid_tok.numel() == 0:
            valid_tok = torch.tensor([0], device=device, dtype=torch.long)

        k_merge = int(torch.clamp(torch.round(out["budget_merge"][i]), 0.0, float(min(n_child, k_budget))).item())
        k_eff = int(torch.clamp(torch.round(out["budget_eff"][i]), 0.0, float(min(n_gen, k_budget - k_merge))).item())
        k_keep = max(1, min(k_budget - k_merge - k_eff, int(valid_tok.numel())))

        def _pick(indices: torch.Tensor, k: int) -> torch.Tensor:
            if k <= 0 or indices.numel() == 0:
                return indices[:0]
            k = min(k, int(indices.numel()))
            vals = w[i, indices]
            top = torch.topk(vals, k=k, largest=True).indices
            return indices[top]

        picked = torch.cat([
            _pick(valid_tok, k_keep),
            _pick(idx_child, k_merge),
            _pick(idx_gen, k_eff),
        ])

        short = k_budget - int(picked.numel())
        if short > 0:
            order = torch.argsort(w[i], descending=True)
            if picked.numel() > 0:
                taken = torch.zeros(N, dtype=torch.bool, device=device)
                taken[picked] = True
                order = order[~taken[order]]
            picked = torch.cat([picked, order[:short]])
...
def compute_reconstruction_losses(
    ...
    w_soft = out["cand_weights"].clamp(0.0, 1.0)

    # Inference-aligned hard selection (straight-through estimator).
    hard_mask = _build_hard_selection_mask(out, mask_hlt, max_tokens=int(const_off.shape[1]))
    hard_alpha = float(loss_cfg.get("hard_select_alpha", 1.0))
    w_target = (1.0 - hard_alpha) * w_soft + hard_alpha * hard_mask
    w = w_soft + (w_target - w_soft).detach()

    L = mask_hlt.shape[1]
    n_child = out["child_weight"].shape[1]
    n_gen = out["gen_weight"].shape[1]
    w_tok = w[:, :L]
    w_child = w[:, L:L + n_child]
    w_gen = w[:, L + n_child:L + n_child + n_gen]
    ...
    loss_budget_core = (
        ...
        + F.smooth_l1_loss(budget_total - hlt_count, true_added)
        + F.smooth_l1_loss(budget_merge + budget_eff, true_added)
    )

    # Internal consistency between branch masses and branch budgets.
    ...
    eff_floor_frac = float(loss_cfg.get("eff_floor_frac", 0.10))
    eff_min = eff_floor_frac * true_added * (0.5 + 0.5 * low_frac)
    has_added = (true_added > 0.5).float()
    loss_eff_floor = (F.relu(eff_min - pred_added_eff) * has_added).sum() / (has_added.sum() + eps)

    loss_budget = (
        loss_budget_core
        + float(loss_cfg.get("w_branch_consistency", 0.0)) * loss_branch_consistency
        + float(loss_cfg.get("w_latent_prior", 0.0)) * loss_latent_prior
        + float(loss_cfg.get("w_eff_floor", 0.0)) * loss_eff_floor
    )

    # Sparsity regularization (avoid gratuitous split/gen actions).
    loss_sparse_split = w_child.mean()
    loss_sparse_gen = w_gen.mean()
    loss_sparse = loss_sparse_split + loss_sparse_gen

    # Locality regularization.
    split_delta = out["split_delta"]
    split_step = torch.sqrt(split_delta[..., 1].pow(2) + split_delta[..., 2].pow(2) + 1e-8)
    split_w = w_child.reshape(split_step.shape)
    ...
    return {
        ...
        "latent_prior": loss_latent_prior,
        "local": loss_local,
    }


def stage_control(epoch: int, cfg: Dict, base_loss_cfg: Dict) -> Tuple[float, Dict]:
    """Three-phase staged training with stage-dependent loss weighting."""
    s1 = int(cfg["stage1_epochs"])
    s2 = int(cfg["stage2_epochs"])
    loss_cfg = dict(base_loss_cfg)
    ...
    if epoch < s1:
        ...
        loss_cfg["w_sparse_split"] = float(base_loss_cfg.get("w_sparse_split", 0.0)) * 1.00
        loss_cfg["w_sparse_gen"] = float(base_loss_cfg.get("w_sparse_gen", 0.0)) * 1.00
        loss_cfg["w_local"] = float(base_loss_cfg.get("w_local", 0.0)) * 1.00
        ...
        loss_cfg["w_sparse_gen"] = float(base_loss_cfg.get("w_sparse_gen", 0.0)) * 0.80
        loss_cfg["w_local"] = float(base_loss_cfg.get("w_local", 0.0)) * 1.10
        loss_cfg["hard_select_alpha"] = 1.00
    return sc, loss_cfg
...
def reconstruct_dataset(
    ...
    soft_count_total = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    soft_count_merge = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    soft_count_eff = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    hard_count_total = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    hard_count_keep = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    hard_pt_keep = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    hard_pt_merge = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    hard_pt_eff = np.zeros(feat_hlt.shape[0], dtype=np.float32)
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="ReconstructAll"):
            ...
            for i in range(bsz):
                ...
                hard_count_total[offset + i] = float(n)
                hard_count_keep[offset + i] = float(np.sum(sel_arr < n_tok))
                idx_keep = sel_arr[sel_arr < n_tok]
                idx_merge = sel_arr[(sel_arr >= n_tok) & (sel_arr < gen_start)]
                idx_eff = sel_arr[sel_arr >= gen_start]
                if idx_keep.size > 0:
                    hard_pt_keep[offset + i] = float(np.sum(cand[i, idx_keep, 0]))
                if idx_merge.size > 0:
                    hard_pt_merge[offset + i] = float(np.sum(cand[i, idx_merge, 0]))
                if idx_eff.size > 0:
                    hard_pt_eff[offset + i] = float(np.sum(cand[i, idx_eff, 0]))
                ...
                soft_count_total[offset + i] = float(np.sum(w[i]))
                soft_count_merge[offset + i] = float(np.sum(w[i, n_tok:n_tok + n_child]))
                soft_count_eff[offset + i] = float(np.sum(w[i, gen_start:gen_start + n_gen]))
...
def plot_response_vs_score_bins(
    save_root: Path,
    name: str,
    scores: np.ndarray,
    pt_truth: np.ndarray,
    ...
    pt_truth = np.asarray(pt_truth, dtype=np.float64).reshape(-1)
    pt_hlt = np.asarray(pt_hlt, dtype=np.float64).reshape(-1)
    pt_reco = np.asarray(pt_reco, dtype=np.float64).reshape(-1)

    ratio_hlt = pt_hlt / np.maximum(pt_truth, eps)
    ratio_reco = pt_reco / np.maximum(pt_truth, eps)

    q_edges = np.quantile(scores, np.linspace(0.0, 1.0, n_bins + 1))
        ...
        records.append(
            {
                "score_low": float(lo),
                "score_high": float(hi),
                "score_mean": float(np.mean(scores[sel])),
                "count": n,
                "hlt_resp_mean": float(np.mean(ratio_hlt[sel])),
                "hlt_resp_std": float(np.std(ratio_hlt[sel])),
                "reco_resp_mean": float(np.mean(ratio_reco[sel])),
                "reco_resp_std": float(np.std(ratio_reco[sel])),
            }
        )

    out_json = save_root / f"response_vs_score_{name}.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump({"model": name, "bins": records}, f, indent=2)

    if len(records) > 0:
        x = np.array([r["score_mean"] for r in records], dtype=np.float64)
        h_mean = np.array([r["hlt_resp_mean"] for r in records], dtype=np.float64)
        h_std = np.array([r["hlt_resp_std"] for r in records], dtype=np.float64)
        r_mean = np.array([r["reco_resp_mean"] for r in records], dtype=np.float64)
        r_std = np.array([r["reco_resp_std"] for r in records], dtype=np.float64)

        plt.figure(figsize=(8.8, 5.2))
        plt.plot(x, h_mean, "o-", color="steelblue", label="HLT response mean")
        plt.fill_between(x, h_mean - h_std, h_mean + h_std, color="steelblue", alpha=0.18, linewidth=0)
        plt.plot(x, r_mean, "s-", color="darkorange", label="Corrected response mean")
        plt.fill_between(x, r_mean - r_std, r_mean + r_std, color="darkorange", alpha=0.18, linewidth=0)
        plt.axhline(1.0, color="black", linestyle=":", linewidth=1.2)
        plt.xlabel(f"{name} score (quantile-bin mean)")
        plt.ylabel("pT response (reco/truth)")
        plt.grid(True, alpha=0.3)
        plt.legend(frameon=False)
        plt.tight_layout()
        plt.savefig(save_root / f"response_vs_score_{name}.png", dpi=300)
        plt.close()
    return {"bins": records}
...
def run_extended_diagnostics(
    ...
    created_merge_count: np.ndarray,
    created_eff_count: np.ndarray,
    soft_count_total: np.ndarray,
    soft_count_merge: np.ndarray,
    soft_count_eff: np.ndarray,
    hard_count_total: np.ndarray,
    hard_count_keep: np.ndarray,
    hard_pt_keep: np.ndarray,
    hard_pt_merge: np.ndarray,
    hard_pt_eff: np.ndarray,
    labels: np.ndarray,
    ...
    # Hard-vs-soft selection gap diagnostics (train/inference mismatch tracker).
    soft_total = soft_count_total.astype(np.float32)
    soft_merge = soft_count_merge.astype(np.float32)
    soft_eff = soft_count_eff.astype(np.float32)
    hard_total = hard_count_total.astype(np.float32)
    hard_merge = created_merge_count.astype(np.float32)
    hard_eff = created_eff_count.astype(np.float32)
    gap_total = hard_total - soft_total
    gap_merge = hard_merge - soft_merge
    gap_eff = hard_eff - soft_eff

    # Branch-level hard-selected pT composition.
    hard_pt_keep = hard_pt_keep.astype(np.float32)
    hard_pt_merge = hard_pt_merge.astype(np.float32)
    hard_pt_eff = hard_pt_eff.astype(np.float32)
    hard_pt_total = np.maximum(hard_pt_keep + hard_pt_merge + hard_pt_eff, 1e-8)
    hard_pt_keep_frac = hard_pt_keep / hard_pt_total
    hard_pt_merge_frac = hard_pt_merge / hard_pt_total
    hard_pt_eff_frac = hard_pt_eff / hard_pt_total
    ...
    # Split-count confusion in coarse bins: 0 / 1 / 2 / >=3.
    true_bin = np.clip(np.rint(t_add), 0, 3).astype(np.int32)
    pred_bin = np.clip(np.rint(p_add_sel), 0, 3).astype(np.int32)
    cm = np.zeros((4, 4), dtype=np.int64)
    for tb, pb in zip(true_bin, pred_bin):
        cm[int(tb), int(pb)] += 1
    cm_norm = cm / np.maximum(cm.sum(axis=1, keepdims=True), 1)

    plt.figure(figsize=(6.8, 5.6))
    im = plt.imshow(cm_norm, cmap="Blues", vmin=0.0, vmax=1.0, aspect="auto")
    plt.colorbar(im, fraction=0.046, pad=0.04, label="Row-normalized fraction")
    ticks = ["0", "1", "2", "3+"]
    plt.xticks(np.arange(4), ticks)
    plt.yticks(np.arange(4), ticks)
    plt.xlabel("Predicted selected children/bin")
    plt.ylabel("True added/bin")
    plt.title("Parent Split-Count Confusion (selected)")
    for r in range(4):
        for c in range(4):
            plt.text(c, r, f"{cm_norm[r, c]:.2f}", ha="center", va="center", color="black", fontsize=9)
    plt.tight_layout()
    plt.savefig(save_root / "split_confusion_matrix.png", dpi=300)
    plt.close()

    # Hard-vs-soft budget gap histogram.
    gmin = float(np.floor(min(gap_total.min(), gap_merge.min(), gap_eff.min())))
    gmax = float(np.ceil(max(gap_total.max(), gap_merge.max(), gap_eff.max())))
    bins_g = np.linspace(gmin - 0.5, gmax + 0.5, num=max(25, int(gmax - gmin + 2)))
    plt.figure(figsize=(8.5, 5))
    plt.hist(gap_total, bins=bins_g, density=True, histtype="step", linewidth=2, color="slateblue", label="hard-soft total")
    plt.hist(gap_merge, bins=bins_g, density=True, histtype="step", linewidth=2, color="darkorange", label="hard-soft merge")
    plt.hist(gap_eff, bins=bins_g, density=True, histtype="step", linewidth=2, color="forestgreen", label="hard-soft eff")
    plt.axvline(0.0, color="gray", linestyle=":", linewidth=1)
    plt.xlabel("Hard count - Soft count")
    plt.ylabel("Density")
    plt.grid(True, alpha=0.3)
    plt.legend(frameon=False)
    plt.tight_layout()
    plt.savefig(save_root / "hard_soft_budget_gap_hist.png", dpi=300)
    plt.close()
    ...
    summary = {
        ...
        "hard_soft_total_mae": float(np.abs(gap_total).mean()),
        "hard_soft_merge_mae": float(np.abs(gap_merge).mean()),
        "hard_soft_eff_mae": float(np.abs(gap_eff).mean()),
        "hard_soft_total_bias": float(gap_total.mean()),
        "hard_soft_merge_bias": float(gap_merge.mean()),
        "hard_soft_eff_bias": float(gap_eff.mean()),
        "hard_pt_keep_frac_mean": float(hard_pt_keep_frac.mean()),
        "hard_pt_merge_frac_mean": float(hard_pt_merge_frac.mean()),
        "hard_pt_eff_frac_mean": float(hard_pt_eff_frac.mean()),
        "split_confusion_matrix_counts": cm.tolist(),
        "split_confusion_matrix_row_norm": cm_norm.tolist(),
        "by_class": by_class,
    }
    ...
    lines = [
        ...
        f"Hard-soft count MAE (total/merge/eff): {summary['hard_soft_total_mae']:.4f} / {summary['hard_soft_merge_mae']:.4f} / {summary['hard_soft_eff_mae']:.4f}",
        f"Hard-soft count bias (total/merge/eff): {summary['hard_soft_total_bias']:.4f} / {summary['hard_soft_merge_bias']:.4f} / {summary['hard_soft_eff_bias']:.4f}",
        f"Hard-selected pT fractions keep/merge/eff: {summary['hard_pt_keep_frac_mean']:.4f} / {summary['hard_pt_merge_frac_mean']:.4f} / {summary['hard_pt_eff_frac_mean']:.4f}",
    ]
...
def main():
    ...
    # ====================================================================== #
    print("\n" + "=" * 70)
    print("STEP 3: OFFLINE RECONSTRUCTOR (NO CONST MAP, NO PRIV BUDGET SUP)")
    print("=" * 70)
    ...
    # Response vs classifier score diagnostics (where kinematic issues correlate with score).
    score_resp_baseline = plot_response_vs_score_bins(
        save_root=save_root,
        name="baseline",
        scores=preds_baseline,
        pt_truth=pt_truth_test,
        pt_hlt=pt_hlt_test,
        pt_reco=pt_reco_test,
    )
    score_resp_dual_kd = plot_response_vs_score_bins(
        save_root=save_root,
        name="dual_kd",
        scores=preds_dual_kd,
        pt_truth=pt_truth_test,
        pt_hlt=pt_hlt_test,
        pt_reco=pt_reco_test,
    )
    score_resp_dual_flag_kd = plot_response_vs_score_bins(
        save_root=save_root,
        name="dual_flag_kd",
        scores=preds_dual_flag_kd,
        pt_truth=pt_truth_test,
        pt_hlt=pt_hlt_test,
        pt_reco=pt_reco_test,
    )
    print(
        f"\nResponse-vs-score bins saved: baseline={len(score_resp_baseline['bins'])}, "
        f"dual_kd={len(score_resp_dual_kd['bins'])}, dual_flag_kd={len(score_resp_dual_flag_kd['bins'])}"
    )
    ...
    extended_diag = run_extended_diagnostics(
        ...
        soft_count_total=soft_count_total[test_idx],
        soft_count_merge=soft_count_merge[test_idx],
        soft_count_eff=soft_count_eff[test_idx],
        hard_count_total=hard_count_total[test_idx],
        hard_count_keep=hard_count_keep[test_idx],
        hard_pt_keep=hard_pt_keep[test_idx],
        hard_pt_merge=hard_pt_merge[test_idx],
        hard_pt_eff=hard_pt_eff[test_idx],
    ...
    print(
        f"  Hard-soft count MAE (tot/merge/eff): {extended_diag['hard_soft_total_mae']:.3f} / "
        f"{extended_diag['hard_soft_merge_mae']:.3f} / {extended_diag['hard_soft_eff_mae']:.3f}"
    )
    ...
    np.savez(
        ...
        diag_hard_soft_total_mae=extended_diag["hard_soft_total_mae"],
        diag_hard_soft_merge_mae=extended_diag["hard_soft_merge_mae"],
        diag_hard_soft_eff_mae=extended_diag["hard_soft_eff_mae"],
        diag_hard_pt_keep_frac=extended_diag["hard_pt_keep_frac_mean"],
        diag_hard_pt_merge_frac=extended_diag["hard_pt_merge_frac_mean"],
        diag_hard_pt_eff_frac=extended_diag["hard_pt_eff_frac_mean"],
    ...
    np.savez_compressed(
        ...
        soft_count_total=soft_count_total.astype(np.float32),
        soft_count_merge=soft_count_merge.astype(np.float32),
        soft_count_eff=soft_count_eff.astype(np.float32),
        hard_count_total=hard_count_total.astype(np.float32),
        hard_count_keep=hard_count_keep.astype(np.float32),
        hard_count_merge=created_merge_count.astype(np.float32),
        hard_count_eff=created_eff_count.astype(np.float32),
        hard_pt_keep=hard_pt_keep.astype(np.float32),
        hard_pt_merge=hard_pt_merge.astype(np.float32),
        hard_pt_eff=hard_pt_eff.astype(np.float32),
