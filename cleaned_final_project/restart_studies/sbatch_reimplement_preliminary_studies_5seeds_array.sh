...
#SBATCH --array=0-4
#SBATCH --output=restart_studies/logs/prelim_reimpl5_%A_%a.out
#SBATCH --error=restart_studies/logs/prelim_reimpl5_%A_%a.err
...
# Exactly 5 seeds by design for this stage.
...
RUN_NAME="${RUN_BASENAME}_seed${SEED}"
...
if [[ -z "${SINGLE_SEED_SCRIPT}" ]]; then
  echo "ERROR: Could not find single-seed sbatch script." >&2
...
echo "============================================================"
echo "Preliminary Study Reimplementation (5-seed array task)"
