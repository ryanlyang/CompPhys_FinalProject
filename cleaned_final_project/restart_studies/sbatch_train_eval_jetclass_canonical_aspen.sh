#!/usr/bin/env bash
#SBATCH --job-name=canonClean
#SBATCH --partition=tier3
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=4
#SBATCH --mem=96G
#SBATCH --time=07:00:00
#SBATCH --output=restart_studies/logs/canon_clean_%j.out
#SBATCH --error=restart_studies/logs/canon_clean_%j.err
...
SCRIPT_CANDIDATES=(
  "${WORKDIR}/restart_studies/train_eval_jetclass_canonical_aspen.py"
  "${PROJECT_ROOT}/restart_studies/train_eval_jetclass_canonical_aspen.py"
  "/home/ryreu/atlas/CompPhys_FinalProject/restart_studies/train_eval_jetclass_canonical_aspen.py"
  "/home/ryan/Documents/School/CompPhys/new_final_project/restart_studies/train_eval_jetclass_canonical_aspen.py"
)
SCRIPT_PATH=""
...
if [[ -z "${SCRIPT_PATH}" ]]; then
  echo "ERROR: train_eval_jetclass_canonical_aspen.py not found." >&2
...
RUN_NAME="${RUN_NAME:-jetclass_canonical_clean_seed52_aspen1M}"
SEED="${SEED:-52}"
...
CANONICAL_BACKEND_RESOLVED=""
for b in "${BACKEND_CANDIDATES[@]}"; do
  if [[ -f "${b}" ]]; then
    CANONICAL_BACKEND_RESOLVED="${b}"
    break
  fi
done

if [[ -z "${CANONICAL_BACKEND_RESOLVED}" ]]; then
  echo "ERROR: canonical backend evaluate_jetclass_hlt_teacher_baseline.py not found." >&2
  echo "Checked: ${BACKEND_CANDIDATES[*]}" >&2
  exit 2
fi
...
echo "[preflight] CANONICAL_BACKEND=${CANONICAL_BACKEND_RESOLVED}"
...
CMD=(
  ...
  --canonical_backend_py "${CANONICAL_BACKEND_RESOLVED}"
...
echo "============================================================"
echo "Canonical Clean JetClass -> Aspen Evaluation (Single Seed)"
...
echo "  ${OUTPUT_ROOT}/${RUN_NAME}/aspen_shift_metrics.json"
echo "  ${OUTPUT_ROOT}/${RUN_NAME}/interpretability_per_fraction.csv"
echo "  ${OUTPUT_ROOT}/${RUN_NAME}/method_effectiveness_summary.csv"
echo "  ${OUTPUT_ROOT}/${RUN_NAME}/clean_baseline_best.pt"
